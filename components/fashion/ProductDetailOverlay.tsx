...
export default function ProductDetailOverlay({ product, onClose, onAddToCart }: ProductDetailOverlayProps) {
  // Guard against undefined colors array before mapping
  ...
  // 2. Data Sync Logic (Likes & Reviews)
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-0 md:p-4">
      ...
      <div className="relative bg-white w-full h-full md:h-auto md:max-h-[93vh] md:max-w-5xl md:rounded-xl overflow-hidden flex flex-col md:flex-row shadow-2xl border-none outline-none">
        ...
        <div className="w-full md:w-1/2 flex flex-col h-full bg-white border-none outline-none overflow-hidden">
          <div className="flex-1 overflow-y-auto pl-4 pr-2 md:p-10 custom-scrollbar border-none outline-none bg-white">
            <div className="mb-1">
              <p className="text-emerald-600 text-[10px] font-black uppercase tracking-widest mb-1">{product.category}</p>
              <h1 className="text-xl md:text-2xl font-black text-gray-900 leading-tight italic uppercase">{product.name}</h1>
            ...
            <div className="mb-4">
              ...
              <div className="flex gap-3">
                {product.colors.map((color, index) => (
                  <button key={index} onClick={() => { setSelectedColorIndex(index); setSelectedImage(index); }} className={`p-1 rounded-full border-2 transition-all ${selectedColorIndex === index ? 'border-emerald-600 scale-110' : 'border-transparent'}`}><div className="w-5 h-5 rounded-full shadow-inner border border-black/5" style={{ backgroundColor: color.code }} /></button>
                ))}
              </div>
            </div>
