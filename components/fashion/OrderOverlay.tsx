...
export default function OrderOverlay({ product, onClose, initialColorIndex = 0 }: OrderOverlayProps) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-end md:items-center justify-center p-0 md:p-2">
      <div className="absolute inset-0 bg-black/60 backdrop-blur-sm transition-opacity" onClick={onClose} />

      {/* Ultra Compact Modal */}
      <div className="relative bg-white rounded-t-3xl md:rounded-2xl max-w-sm w-full shadow-2xl animate-in slide-in-from-bottom md:zoom-in-95 duration-300">
        ...
        <div className="p-4">
          {/* Mini Product Summary - Half Size */}
          <div className="flex gap-3 p-2 bg-emerald-50 rounded-xl mb-3">
            ...
            <div className="flex flex-col justify-center min-w-0">
              ...
              <p className="font-black text-emerald-700 text-sm">${product.price.toFixed(2)}</p>
            </div>
          </div>

          {/* Color Selection - Tiny circles */}
          <div className="mb-3">
            <h4 className="text-[9px] font-black uppercase text-emerald-700 mb-2 tracking-widest">Color</h4>
          ...
          {/* Quantity & Actions - Minimal Vertical Space */}
          <div className="flex items-center justify-between pt-3 border-t border-gray-50 gap-4">
            <div className="flex items-center bg-emerald-50 rounded-lg p-0.5 border border-gray-200">
              <button onClick={() => setQuantity(Math.max(1, quantity - 1))} className="w-7 h-7 flex items-center justify-center text-gray-500"><FaMinus size={8} /></button>
              <span className="w-6 text-center font-bold text-xs">{quantity}</span>
              <button onClick={() => setQuantity(quantity + 1)} className="w-7 h-7 flex items-center justify-center text-gray-500"><FaPlus size={8} /></button>
            </div>
            
            <div className="text-right">
               <p className="text-[9px] text-gray-400 font-bold uppercase tracking-tighter">Total</p>
               <p className="text-lg font-black text-gray-900 leading-none">${totalAmount.toFixed(2)}</p>
            </div>
          </div>
