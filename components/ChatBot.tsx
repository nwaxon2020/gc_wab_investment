...
export default function ChatBot() {
 ...
 if (!isOpen) {
  return (
   <button onClick={() => setIsOpen(true)} className="fixed bottom-6 right-6 z-[100] bg-emerald-700 text-white p-4 rounded-full shadow-xl hover:scale-110 transition-transform active:scale-95">
 ...
 return (
  <div className="fixed inset-0 md:inset-auto md:bottom-24 md:right-6 md:left-auto z-[200] flex items-end justify-center pointer-events-none">
   {/* Chat Window Container */}
   <div className="w-full h-full md:w-[720px] md:h-[550px] md:max-h-[80vh] bg-[#0f3d2e] md:rounded-3xl shadow-2xl overflow-hidden flex flex-row relative pointer-events-auto">
    ...
    {!user ? (
     ...
     <>
      {/* Sidebar (Chat List) */}
      <div className={`w-full md:w-64 bg-[#0b2f23] border-r border-gray-200 flex-col shrink-0 ${mobileView === 'chat' && isAdmin ? 'hidden md:flex' : 'flex'} ${!isAdmin && 'hidden md:flex'}`}>
       <div className="p-4 bg-emerald-800 text-white font-bold flex justify-between items-center shrink-0">
        <span className="text-xs uppercase tracking-widest">{isAdmin ? "Clients" : "Support"}</span>
        <button onClick={closeChat} className="hover:rotate-90 transition-transform"><XMarkIcon className="w-6 h-6 text-white" /></button>
       </div>

       <div className="mt-8 bg-green-50 flex-1 overflow-y-auto">
        {!isAdmin ? (
         <div 
         ...
         <div className="relative h-full">
          {chats.length === 0 && (
           <div className="absolute inset-0 flex flex-col items-center justify-center opacity-20 p-4 text-center">
      ...
      {/* Main Chat Interface */}
      <div className={`flex-1 flex flex-col min-w-0 ${mobileView === 'list' && isAdmin ? 'hidden md:flex' : 'flex'}`}>
        ...
        <div className="flex items-center gap-3">
         ...
         <div className="flex flex-col leading-none">
          <span className="font-bold text-sm">
           {isAdmin ? (chats.find(c => c.id === activeChatId)?.userName || 'Select Client') : 'Concierge Support'}
          </span>
          <span className="text-[9px] text-emerald-300 font-medium uppercase tracking-tighter">Verified Assistant</span>
         </div>
        </div>
        <button onClick={closeChat} className="hover:rotate-90 transition-transform"><XMarkIcon className="w-6 h-6 text-white" /></button>
       </div>

       <div className="flex-1 overflow-y-auto p-4 bg-green-50 space-y-4 relative scroll-smooth">
        {messages.length === 0 && (
         <div className="absolute inset-0 flex flex-col items-center justify-center opacity-20">
           <ChatBubbleLeftRightIcon className="w-16 h-16 text-[#0f3d2e] mb-2" />
           <p className="text-xs font-bold text-[#0f3d2e] uppercase tracking-widest">Start a Conversation</p>
         </div>
        )}

        {messages.map((m, i) => (
         <div key={i} className={`flex flex-col ${m.isAdmin ? 'items-end' : 'items-start'}`}>
          <div className={`max-w-[85%] md:max-w-[75%] p-3 rounded-2xl text-sm ${m.isAdmin ? 'bg-emerald-600 text-white rounded-tr-none shadow-md' : 'bg-white text-[#0f3d2e] border border-emerald-100 rounded-tl-none shadow-sm'}`}>
           {m.text}
          </div>
          {m.isAdmin && isAdmin && (
       ...
       <form onSubmit={send} className="p-3 md:p-4 bg-[#0f3d2e] flex gap-2 shrink-0">
        <input 
          value={text} 
          onChange={e => setText(e.target.value)} 
          className="flex-1 rounded-xl bg-white border-none px-4 py-3 text-sm text-[#0f3d2e] outline-none placeholder:text-gray-400" 
          placeholder="Message Support..." 
        />
        <button className="bg-emerald-600 text-white p-3 rounded-xl hover:bg-emerald-500 transition-colors shadow-lg">
